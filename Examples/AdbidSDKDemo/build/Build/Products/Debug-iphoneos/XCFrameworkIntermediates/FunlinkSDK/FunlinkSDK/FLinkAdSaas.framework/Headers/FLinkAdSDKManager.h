...
typedef NS_ENUM(NSInteger, FLinkGDPRConsentSet) {
    FLinkGDPRConsentSetUnknown = 0,
    FLinkGDPRConsentSetPersonalized,
    FLinkGDPRConsentSetNonpersonalized
};

typedef NS_ENUM(NSInteger, FLinkADEnvironment) {
    FLinkADEnvironmentDebug = 0,   // 内网
    FLinkADEnvironmentChina,
    FLinkADEnvironmentOther,       // 海外
    FLinkADEnvironmentAuto
};

NS_ASSUME_NONNULL_BEGIN

@interface FLinkAdSDKManager : NSObject
...
@property (nonatomic, readonly) FLinkGDPRConsentSet dataConsentSet;
...
/// 海外注册APP id
+ (BOOL)registerAppId:(NSString *)appId Environment:(FLinkADEnvironment)environment;
/// 获取注册的APP id
+ (NSString *)appId;
///执行后需重新进行初始化
+ (void)unRegister;
...
/// 欧盟GDPR合规弹窗
- (void)presentDataConsentDialogInViewController:(UIViewController *)viewController dismissalCallback:(void(^)(FLinkGDPRConsentSet dataConsentSet))dismissCallback;
